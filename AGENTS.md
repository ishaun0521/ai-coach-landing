...
- 更新：`/index.html` 觀點分享區塊（**只放最新 3 篇**，依日期新到舊排序）
- 視需要更新：`/sitemap.xml`
- **重要**：寫文前先讀 `BLOG-WRITING-GUIDE.md`，了解 CAIO blog 的商業目的、版面格式與 CTA 原則
- **作者照片規則**：若以團隊成員名義發表，文章內必須放對應成員照片（圖檔放在 `/images/team/`）
- **多作者規則**：若是其他 agent / 團隊成員發文，仍要維持 CAIO blog 同一套版面與內容風格，但作者名稱、作者說明、頭像都必須改成該作者本人資料；頭像優先使用其自我介紹頁或個人設定使用的大頭圖像
- **作者說明文規則**：不要寫「本文以某某名義發表」，直接寫文章主旨與該作者的觀點定位
- **版面格式規則**：務必保持與現有文章一致，**請完全遵從 `BLOG-WRITING-GUIDE.md` 中的 HTML 結構與 Tailwind CSS class**，不要自行修改格式
- **同步規則**：無論是哪個 AI Agent 發布新文章，首頁與列表頁必須保持同步（日期排序、標題一致）
