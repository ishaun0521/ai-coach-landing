...
補充規則：
- 若文章由不同 agent / 團隊成員名義發表，**作者名稱、職稱、作者說明文、頭像** 都要對應到該作者本人。
- **頭像一律使用該作者自我介紹 / 個人頁使用的大頭圖像**，不要沿用 Shaun 或其他人的照片。
- 文章的觀點、語氣、定位可以維持 CAIO blog 的整體品牌風格，但**署名與人物呈現必須正確對應作者本人**。
- 不要寫「本文以某某名義發表」這種句型，直接寫成該文章的觀點摘要或定位說明即可。
...
- 更新 `/blog/index.html` - 放置**所有文章**（依日期新到舊排序）
- 更新 `/index.html` 的觀點分享區塊 - 只放**最新 3 篇**（依日期新到舊排序）
- 補作者區塊與照片
- 檢查文章內鏈
- 視需要更新 `sitemap.xml`
...
**每次發布新文章時，必須同時更新以下兩個頁面：**

### 14.1 首頁（index.html）
- 位置：`<section class="py-20 bg-slate-50">` 區塊中的「觀點分享」
- 規則：**只顯示最新 3 篇文章**（依日期新到舊排序）
- 格式：簡短標題 + 摘要（約 30-50 字）+ 連結

### 14.2 部落格列表頁（blog/index.html）
- 位置：`<section class="py-14 md:py-16">` 區塊
- 規則：**顯示所有文章**（依日期新到舊排序）
- 格式：完整作者資訊 + 標題 + 摘要預覽 + 連結

### 14.3 同步原則
1. **日期排序**：所有文章必須依 `datePublished` 降序排列（最新在前）
2. **標題一致**：首頁與列表頁使用相同的文章標題，不得自行簡化或修改
3. **完整同步**：無論是哪個 AI Agent 發布新文章，都必須遵守此規範
4. **發布即同步**：新增文章後，必須**立即**更新這兩個頁面

### 14.4 範例

假設新增一篇 2026-03-13 的文章：
...
**列表頁應該顯示：**
1. 2026-03-13 - [最新文章標題] + 完整資訊
2. 2026-03-12 - [第二天新文章] + 完整資訊
3. 2026-03-11 - [第三天新文章] + 完整資訊
4. ...（所有舊文章）

### 14.5 提醒機制

**所有 OpenClaw agents 請注意**：
- 當你發布新文章時，系統會自動提醒你同步更新首頁與列表頁
- 如果忘記同步，可能會導致：
  - 首頁顯示過時文章
  - 列表頁缺少最新文章
  - 文章標題在不同頁面不一致
